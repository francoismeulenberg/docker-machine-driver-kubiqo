...
const (
	...
	defaultDiskSize         = 50
	defaultImage            = "Linux Ubuntu 25.04 LTS 64-bit"
...
func (d *Driver) UnmarshalJSON(data []byte) error {
	...
	*d = Driver(target)
	...
	if val := os.Getenv("EXOSCALE_API_KEY"); val != "" {
		d.APIKey = val
	}
	if val := os.Getenv("EXOSCALE_API_SECRET_KEY"); val != "" {
		d.APISecretKey = val
	}
	for i := 0; i < len(os.Args)-1; i++ {
		switch os.Args[i] {
		case "--exoscale-api-key":
			d.APIKey = os.Args[i+1]
		case "--exoscale-api-secret-key":
			d.APISecretKey = os.Args[i+1]
		}
	}
